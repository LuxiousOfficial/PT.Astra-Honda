<x-layout-user>
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
      <h1 class="fs-3 fw-bold text-danger text-capitalize">Data Skills</h1>
    </div>
      <section class="form-pendaftaran bg-white rounded-3 p-4">
        <div class="container">
            <div class="row text-center">
                <div class="col-md-12 col-12">
                  {{-- <div class="title text-capitalize">
                      <h2 class="fs-1 fw-bold mb-4">{{ $education->university }}</h2>
                  </div> --}}
                  <div class="table-responsive col-xxl-12 col-xl-12 col-md-12 col-sm-12 col-12 small">
                        <table class="table table-sm text-center fs-5">
                          <thead class="fs-4">
                            <tr class="table table-info">
                              <th scope="col">Info</th>
                              <th scope="col">Keterangan</th>
                            </tr>
                          </thead>
                          <tbody>
                          <tr>
                            <td>First Skill</td>
                            <td>{{ $skill->first_skill }}</td>
                          </tr>
                          <tr>
                            <td>First Rate Skill</td>
                            <td>{{ $skill->first_rate }}</td>
                          </tr>
                          <tr>
                            <td>Second Skill</td>
                            <td>{{ $skill->second_skill }}</td>
                          </tr>
                          <tr>
                            <td>Second Rate Skill</td>
                            <td>{{ $skill->second_rate }}</td>
                          </tr>
                          <tr>
                            <td>Third Skill</td>
                            <td>{{ $skill->third_skill }}</td>
                          </tr>
                          <tr>
                            <td>Third Rate Skill</td>
                            <td>{{ $skill->third_rate }}</td>
                          </tr>
                          <tr>
                            <td>Fourth Skill</td>
                            <td>{{ $skill->fourth_skill }}</td>
                          </tr>
                          <tr>
                            <td>Fourth Rate Skill</td>
                            <td>{{ $skill->fourth_rate }}</td>
                          </tr>
                          <tr>
                            <td>Fifth Skill</td>
                            <td>{{ $skill->fifth_skill }}</td>
                          </tr>
                          <tr>
                            <td>Fifth Rate Skill</td>
                            <td>{{ $skill->fifth_rate }}</td>
                          </tr>
                          </tbody>
                        </table>
                      </div>
                    </div>
                    <div class="daftar text-center text-capitalize mt-5">
                      <a href="/user/skills" class="fs-5 fw-bold text-decoration-none py-3 px-3 bg-danger text-light rounded">Back</a>
                    </div>
                </div>
            </div>
        </div>
      </section>
</x-layout-user>